export async function onRequestPost(context) {
  ...
  try {
    const missing = requiredEnv(env, ["RAZORPAY_WEBHOOK_SECRET", "SUPABASE_URL", "SUPABASE_SERVICE_ROLE_KEY"]);
    if (missing.length > 0) {
      return jsonResponse({ success: false, error: "Missing environment variables.", missing }, 500);
    }

    const rawBody = await request.text();
    const razorpaySignature = request.headers.get("x-razorpay-signature");
    if (!razorpaySignature) {
      return jsonResponse({ success: false, error: "Missing Razorpay webhook signature." }, 400);
    }
    ...
    if (!razorpayOrderId) {
      return jsonResponse({ success: true, handled: false, message: "Webhook ignored. No Razorpay order ID found." });
    }

    const patch = buildRegistrationPatch(eventType, payment, order);
    if (!patch) {
      return jsonResponse({ success: true, handled: false, event: eventType, message: `Webhook event ignored: ${eventType}` });
    }
    ...
    return jsonResponse({
      ...
      message: rows.length > 0 ? "Webhook processed and registration updated." : "Webhook processed, but no matching registration row was found.",
    });
  } catch (error) {
    return jsonResponse({ success: false, error: "Webhook server error.", details: error.message }, 500);
  }
}
...
async function updateRegistrationIfExists(env, razorpayOrderId, data) {
  ...
  if (!res.ok) throw new Error(`Supabase webhook update failed: ${text}`);
