export async function onRequestPost(context) {
  ...
  try {
    ...
    const insertRes = await fetch(`${env.SUPABASE_URL}/rest/v1/registrations`, {
      method: "POST",
      headers: {
...
async function findExistingRegistration(env, orderId, paymentId) {
  const url = `${env.SUPABASE_URL}/rest/v1/registrations?select=id,razorpay_order_id,razorpay_payment_id&or=(razorpay_order_id.eq.${orderId},razorpay_payment_id.eq.${paymentId})`;
