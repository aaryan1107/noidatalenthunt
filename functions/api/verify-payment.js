...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const { registration_id, razorpay_order_id, razorpay_payment_id, razorpay_signature, registration } = body;

    if (!registration_id || !razorpay_order_id || !razorpay_payment_id || !razorpay_signature) {
      return jsonResponse({ success: false, error: "Missing payment verification data." }, 400);
    }
    ...
    const expectedSignature = await hmacSha256Hex(`${razorpay_order_id}|${razorpay_payment_id}`, env.RAZORPAY_KEY_SECRET);
    ...
    const expectedCurrency = String(pending?.currency || fallbackRegistration.currency || "INR").toUpperCase();

    if (payment.amount !== expectedAmount || String(payment.currency).toUpperCase() !== expectedCurrency) {
      return jsonResponse(
        {
          success: false,
    ...
    const saved = pending ? await updateRegistration(env, pending.id, row) : await insertRegistration(env, row);

    return jsonResponse({ success: true, message: "Payment verified and registration stored successfully.", registration: saved });
  } catch (error) {
...
function expectedAmountFor(pending, registration) {
  if (Number.isFinite(Number(pending?.amount)) && Number(pending.amount) > 0) return Number(pending.amount);
  if (Number.isFinite(Number(registration?.amount)) && Number(registration.amount) > 0) return Number(registration.amount);
  if (Number.isFinite(Number(registration?.cart_count)) && Number(registration.cart_count) > 0) return Number(registration.cart_count) * PRICE_PER_ITEM;
  if (Array.isArray(registration?.cart_items) && registration.cart_items.length > 0) return registration.cart_items.length * PRICE_PER_ITEM;
...
async function fetchRazorpayPayment(env, paymentId) {
  ...
  if (!res.ok) throw new Error(`Unable to fetch Razorpay payment: ${JSON.stringify(data)}`);
  return data;
}
...
async function captureRazorpayPayment(env, paymentId, amount) {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(`Unable to capture Razorpay payment: ${JSON.stringify(data)}`);
  return data;
}
...
async function findExistingRegistration(env, registrationId, orderId, paymentId) {
  const url = supabaseRestUrl(
    env,
    `registrations?select=*&or=(id.eq.${encodeURIComponent(registrationId)},razorpay_order_id.eq.${encodeURIComponent(orderId)},razorpay_payment_id.eq.${encodeURIComponent(paymentId)})`,
  );
...
async function updateRegistration(env, id, row) {
  ...
  const text = await res.text();
  if (!res.ok) throw new Error(`Supabase update failed: ${text}`);
...
async function insertRegistration(env, row) {
  ...
  const text = await res.text();
  if (!res.ok) throw new Error(`Supabase insert failed: ${text}`);
  return JSON.parse(text)[0];
}
